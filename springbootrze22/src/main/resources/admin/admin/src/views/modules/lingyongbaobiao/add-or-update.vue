    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="设备名称" prop="shebeimingcheng">
          <el-select v-model="ruleForm.shebeimingcheng" placeholder="请选择设备名称">
            <el-option
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="领用数量" prop="lingyongshuliang">
          <el-input v-model="ruleForm.lingyongshuliang" 
              placeholder="领用数量" clearable  :readonly="ro.lingyongshuliang"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="领用数量" prop="lingyongshuliang">
              <el-input v-model="ruleForm.lingyongshuliang" 
                placeholder="领用数量" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.shebeimingcheng = true;
          ...
          if(o=='lingyongshuliang'){
            this.ruleForm.lingyongshuliang = obj[o];
	    this.ro.lingyongshuliang = true;
            continue;
          }
          if(o=='dengjiriqi'){
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `lingyongbaobiao/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.lingyongbaobiaoCrossAddOrUpdateFlag = false;
                  this.parent.search();
